...
class Admin extends QW_Controller
{
    ...
    {
        $add_flag = $this->checkUserButtonPrivilege('admin/add_op');
        $edit_flag = $this->checkUserButtonPrivilege('admin/edit_op');
        $delete_flag = $this->checkUserButtonPrivilege('admin/delete_op');
    ...
    public function show_user()
    {
        ...
        $allusers = $this->Admin->get_users($offset, $eachpage, $sortOrder);
        foreach ($allusers as $key => $value) {
            $value['registered'] = date('Y-m-d H:i:s', $value['registered']);
            $value['last_login'] = date('Y-m-d H:i:s', $value['last_login']);
            $allusers[$key] = $value;
        }
        $infos = array('total' => count($this->Admin->get_users("", "", "")), 'rows' => $allusers);
